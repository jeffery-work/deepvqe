...
from Utility.Metrics import STOI, PESQ, SI_SDR
from Utility.Utils import prepare_empty_dir, ExecutionTime, prepare_device
...
class BaseTrainer:
    def __init__(self, config, resume: bool, model, loss_function, optimizer):
        ...
        print(json5.dumps(config, indent=2, sort_keys=False)) # dumps: convert object to json string

        with open((self.root_dir / f"{time.strftime('%Y-%m-%d-%H-%M-%S')}.json").as_posix(), "w") as handle:
            json5.dump(config, handle, indent=2, sort_keys=False) # dump: convert object to json file
    ...
    def _preload_model(self, model_path):
        ...
        """
        model_path = model_path.expanduser().absolute() # return usr/ absolute path
        assert model_path.exists(), f"Preloaded *.pth file is not exist. Please check the file path: {model_path.as_posix()}"
        model_checkpoint = torch.load(model_path.as_posix(), map_location=self.device) # as_posix:"usr\\model\" to "usr/model/"

        if isinstance(self.model, torch.nn.DataParallel): # is self.model same object as torch.nn.DataParallel
